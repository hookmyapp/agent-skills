...
| Env keys | 5 (WhatsApp): `WEBHOOK_HMAC_SECRET`, `PORT`, `WHATSAPP_API_URL`, `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID` (no `VERIFY_TOKEN` — the sandbox tunnel never issues the verify-GET handshake) | 7 (WhatsApp, from `channels env`): `META_GRAPH_API_URL`, `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_WABA_ID`, `HOOKMYAPP_CHANNEL_ID`, `VERIFY_TOKEN`, `WEBHOOK_HMAC_SECRET` (no `PORT`). Instagram emits the `INSTAGRAM_*` equivalents. |
...
`VERIFY_TOKEN` is a **separate value with one job**: the plain-text body your endpoint returns on the webhook verify GET. It exists only for real channels (auto-generated by the backend; override with `hookmyapp channels webhook set <channel> --verify-token <token>`) — `sandbox env` does not write it at all. Never use it as the HMAC key — channels created before the two were split (2026-06-24) happen to carry the same value in both, but new channels get two independent values.
