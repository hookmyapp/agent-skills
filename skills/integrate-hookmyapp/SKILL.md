...
If `npm` is missing, the CLI path is closed on this machine. Node.js 20+ (which includes npm) is the fix, but do not make it a wall for tasks that do not need it: the [MCP server](references/mcp.md) (`https://api.hookmyapp.com/mcp`) and the [REST API](references/api.md) need no Node and cover account operations — messaging, Instagram publishing and insights, webhook destinations, customers and onboarding links, delivery logs. Two things genuinely need the CLI, and therefore Node: connecting the user's own channel, and `channels listen`. Credentials are not one of them — with an `hmok_` key, REST reads and rotates a channel's `hmat_` token and returns its env set (`GET /meta/channels/{id}/token`, `/token/rotate`, `/env`); only writing them into a `.env` file for the user is CLI work. Say which of the two the user's task falls in, then offer the matching path. If global installs are blocked, stop and ask the user to install the CLI themselves (`npm install -g @gethookmyapp/cli`) or make `hookmyapp` available on PATH another way — do not retry the blocked command. Do not continue with guessed commands or raw API calls just because the CLI is absent.
