...
If `npm` is missing, the CLI path is closed on this machine. Node.js 20+ (which includes npm) is the fix, but do not make it a wall: the [MCP server](references/mcp.md) (`https://api.hookmyapp.com/mcp`) and the [REST API](references/api.md) need no Node and cover every account operation. Offer the install and the no-Node paths together, and let the user pick. If global installs are blocked, stop and ask the user to install the CLI themselves (`npm install -g @gethookmyapp/cli`) or make `hookmyapp` available on PATH another way — do not retry the blocked command. Do not continue with guessed commands or raw API calls just because the CLI is absent.
