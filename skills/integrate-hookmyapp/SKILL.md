...
**Any command failed with a network error** (`ENOTFOUND`, `EAI_AGAIN`, `ETIMEDOUT`, `ECONNREFUSED`, `ECONNRESET`, `ENETUNREACH`, `EHOSTUNREACH`). You are probably in a sandbox without internet access. Ask the user to allow network access, then run the same command again. This comes before every rule below: only if it still fails with network access allowed does it count as a failed install.
...
```bash
mkdir -p ~/.config/hookmyapp && echo "0.9.25" > ~/.config/hookmyapp/skill-version
```
