...
| `publish_instagram_media` | Publish an image, reel, story, or carousel on an Instagram channel: `mediaType`, `imageUrl`/`videoUrl`, `caption`, `children[]` (carousel), `coverUrl`, `shareToFeed`, `trialParams: {graduationStrategy: "manual" | "automatic"}` (reels only — trial reel; rejected on any other `mediaType`). Runs Meta's container → status poll → publish flow and returns `{mediaId, permalink}` |
