...
| `list_instagram_media` | List an Instagram account's own published posts, newest first. Pass `mediaId` instead to read one post with its carousel `children`. **This is where media ids come from** — `get_instagram_insights` and `list_instagram_comments` both need one, and an inbound webhook is the only other source. Stories and tagged posts are NOT available: those edges need a Facebook User token, and Meta states the Instagram-Login setup "cannot access ads or tagging" |
| `get_instagram_mention` | Read a post (`mediaId`) or comment (`commentId`) the account was @mentioned in, and post the reply in the same call with `replyText`. **Instagram cannot list past mentions** — the ids come from the `mentions` webhook, and `/{ig}/mentions` is POST-only. Replying always needs `mediaId`, even for a comment mention |
