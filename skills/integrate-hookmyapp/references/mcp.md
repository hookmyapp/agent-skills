...
| `publish_instagram_media` | Publish an image, reel, story, or carousel on an Instagram channel: `mediaType`, `imageUrl`/`videoUrl`, `caption`, `children[]` (carousel), `coverUrl`, `shareToFeed`, `trialParams: {graduationStrategy}` (reels only — trial reel; rejected on any other `mediaType`). Runs Meta's container → status poll → publish flow and returns `{mediaId, permalink}` |
