...
| `404` on Meta's webhook verify GET | Your server isn't serving the verify path | Ensure your server responds to `GET /webhook` (default) with `VERIFY_TOKEN` as plain-text body (real channels only — the sandbox tunnel never issues the verify GET). |
