...
| `--code <code>` | string | no | — | Skip the prompt and verify with a code already in hand |

**Arguments:** `<phone>` — international format, e.g. `+14155552671`. A national number (`07700900123`) is rejected before anything is sent.
...
- The code is delivered to the human's phone, not to the terminal — there is no way to read it from the CLI. Ask them for it.
