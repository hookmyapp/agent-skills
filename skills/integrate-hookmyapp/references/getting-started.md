...
If the install fails because `npm` is missing, offer both ways forward: install Node.js 20+ (which includes npm) for the CLI path, or — when the task is account operations rather than provisioning — skip Node and drive it through the [MCP server](mcp.md) or the [REST API](api.md). Connecting the user's OWN channel (`channels connect`) and anything that writes a file or holds a tunnel open (`channels listen`, `sandbox listen`, `channels env --write`, `sandbox env`) are CLI-only and still need Node, as are a few others with no remote equivalent (`sandbox stop`, Instagram sandbox replies) — confirm the operation is in the [MCP tool table](mcp.md#tools) or the [REST endpoint map](api.md) before promising the no-Node path. End-customer onboarding is not in that group: `create_onboarding_link` / `POST /org/onboarding-links` mint the browser link over MCP or REST. `hmat_` tokens and env values are likewise readable over REST with an `hmok_` key. If the install fails with `EACCES` (permission denied) on npm's global folder, do not stop: apply the no-sudo recovery in [SKILL.md](../SKILL.md#skill-setup-run-before-any-cli-command) (move npm's global folder to `~/.npm-global`, persist PATH, install again). If the install fails with a network error (`ENOTFOUND`, `EAI_AGAIN`, `ETIMEDOUT`, `ECONNREFUSED`, `ECONNRESET`, `ENETUNREACH`, `EHOSTUNREACH`), ask the human to allow network access for this sandbox and retry. If global installs are blocked for any other reason, ask the human to install the CLI themselves or make `hookmyapp` available on PATH another way — do not retry the blocked command.
